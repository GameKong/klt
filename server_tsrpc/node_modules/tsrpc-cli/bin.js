#!/usr/bin/env node
/*!
 * TSRPC Command-line Interface v2.4.5
 * -----------------------------------------
 * MIT LICENSE
 ...
 * 引入 TS 模块，但不缓存
 */
function importTS(modulePath, ignoreTypeError) {
...
async function ensureSymlinks(confs, logger, isElevate) {
    ...
    for (let conf of confs) {
        ...
        // 没有现成的，重建（重建前先清理）
        await fs__default$1["default"].rm(dst, { recursive: true, force: true });
        // 按相对目录创建
        const relativeSrc = path__default["default"].relative(dstParent, src);
        // Try first time
        let err = await fs__default$1["default"].symlink(relativeSrc, dst, createJunction ? 'junction' : 'dir').catch(e => e);
        // Windows 下无权限，尝试提权
        if (!isElevate && process.platform === 'win32' && (err === null || err === void 0 ? void 0 : err.code) === 'EPERM') {
            ...
            if (elevateResult === false && createJunction) {
                err = await fs__default$1["default"].symlink(relativeSrc, dst, 'junction').catch(e => e);
            }
        }
        // Fail
        if (err) {
...
async function isSymbolicLink(dst, src) {
    // dst 已存在 Symlink
    if (await fs__default$1["default"].lstat(dst).then(v => v.isSymbolicLink()).catch(e => false)) {
        // 检查 symlink 的 destination 是否正确
        const oldSymlinkPath = path__default["default"].resolve(path__default["default"].dirname(dst), await fs__default$1["default"].readlink(dst));
        // 是同款
...
async function syncByConfig(syncConfig, logger) {
    ...
    for (let item of syncConfig) {
        if (item.type === 'copy') {
            ...
            await copyDirReadonly(item.from, item.to, !!item.clean, (_a = item.readonly) !== null && _a !== void 0 ? _a : true, logger);
            CliUtil.done(true);
        }
    }
    // Symlinks
...
async function copyDirReadonly(src, dst, clean, readonly, logger) {
    // Clean
    if (clean) {
...
async function cmdDev(options) {
    ...
    if (conf.sync) {
        ...
        for (let i = 0; i < copyConfs.length; ++i) {
            ...
            if (lstat.isSymbolicLink() || lstat.isFile()) {
                ...
                await copyDirReadonly(conf.from, conf.to, !!conf.clean, (_p = conf.readonly) !== null && _p !== void 0 ? _p : true, options.config.verbose ? console : undefined);
    ...
    if (autoProto && conf.proto) {
        for (let confItem of conf.proto) {
            ...
            delayWatch({
                matches: (_q = confItem.watch) !== null && _q !== void 0 ? _q : watchFiles,
                ignore: [confItem.output, ...(confItem.compatible ? [confItem.compatible] : []), ...((_r = confItem.ignore) !== null && _r !== void 0 ? _r : [])],
                ...
                delay: (_t = (_s = conf.dev) === null || _s === void 0 ? void 0 : _s.delay) !== null && _t !== void 0 ? _t : DEFAULT_DELAY,
    ...
    // Auto Copy
    if (autoSync) {
        (_u = conf.sync) === null || _u === void 0 ? void 0 : _u.forEach((confItem, idx) => {
            var _a, _b;
            ...
            delayWatch({
                ...
                onTrigger: async (eventName, filepath, stats) => {
                    ...
                    // 仅第一次全量
                    if (!isInited) {
                        await copyDirReadonly(confItem.from, confItem.to, !!confItem.clean, (_a = confItem.readonly) !== null && _a !== void 0 ? _a : true, options.config.verbose ? console : undefined);
    ...
    delayWatch({
        ...
        delay: (_w = (_v = conf.dev) === null || _v === void 0 ? void 0 : _v.delay) !== null && _w !== void 0 ? _w : DEFAULT_DELAY,
        watchId: 'DEV_SERVER',
    });
    startDevServer();
}
...
async function fillNewPtlOrMsg(filepath, confItem, parsed) {
    ...
    if (parsed.type === 'Ptl') {
        await fs__default$1["default"].writeFile(filepath, getPtlTemplate(confItem.ptlTemplate)(parsed.basename, filepath, confItem.ptlDir), 'utf-8');
    }
    else if (parsed.type === 'Msg') {
        await fs__default$1["default"].writeFile(filepath, getMsgTemplate(confItem.msgTemplate)(parsed.basename, filepath, confItem.ptlDir), 'utf-8');
    }
    return true;
}
...
function getPtlTemplate(ptlTemplate) {
    if (!ptlTemplate) {
...
function getMsgTemplate(msgTemplate) {
    if (!msgTemplate) {
...
async function generate(ptlDir, outDir, ignore, verbose, resolveModule) {
    // Generate proto
...
function cmdShowHelp() {
    showLogo();
    console.log(chalk__default["default"].green(formatStr(i18n.welcome, { version: '2.4.5' })));
    console.log('\n' + i18n.help);
    console.log('\n' + i18n.example);
}
