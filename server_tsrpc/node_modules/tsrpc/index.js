...
class ApiCall extends BaseCall {
    constructor(options, logger) {
        super(options, logger !== null && logger !== void 0 ? logger : new PrefixLogger({
            ...
            prefixs: [`${chalk__default["default"].cyan.underline(`[Api:${options.service.name}]`)}${options.sn !== undefined ? chalk__default["default"].gray(` SN=${options.sn}`) : ''}`]
        }));
        this.type = 'api';
        this.sn = options.sn;
    ...
    async _prepareReturn(ret) {
        ...
        if (ret.isSucc) {
            this.logger.log(chalk__default["default"].green('[ApiRes]'), `${this.usedTime}ms`, this.server.options.logResBody ? ret.res : '');
        }
        else {
            if (ret.err.type === tsrpcProto.TsrpcErrorType.ApiError) {
                this.logger.log(chalk__default["default"].red('[ApiErr]'), `${this.usedTime}ms`, ret.err, 'req=', this.req);
            }
            else {
                this.logger.error(chalk__default["default"].red('[ApiErr]'), `${this.usedTime}ms`, ret.err, 'req=', this.req);
            }
        }
        // Do send!
        this._return = ret;
...
class BaseConnection {
    ...
    async sendMsg(msgName, msg) {
        ...
        // Do send!
        this.server.options.logMsg && this.logger.log(chalk__default["default"].cyan.underline(`[Msg:${msgName}]`), chalk__default["default"].green('[SendMsg]'), msg);
...
class MsgCall extends BaseCall {
    constructor(options, logger) {
        super(options, logger !== null && logger !== void 0 ? logger : new PrefixLogger({
            logger: options.conn.logger,
            prefixs: [chalk__default["default"].cyan.underline(`[Msg:${options.service.name}]`)]
        }));
        this.type = 'msg';
...
class HttpConnection extends BaseConnection {
    constructor(options) {
        super(options, new PrefixLogger({
            ...
            prefixs: [chalk__default["default"].gray(`${options.ip} #${options.id}`)]
        }));
        this.type = 'SHORT';
...
class InnerConnection extends BaseConnection {
    constructor(options) {
        super(options, new PrefixLogger({
            logger: options.server.logger,
            prefixs: [chalk__default["default"].gray(`Inner #${options.id}`)]
        }));
        this.type = 'SHORT';
...
class BaseServer {
    ...
    constructor(proto, options) {
        ...
        this.tsbuffer = new tsbuffer.TSBuffer({
            ...
            ...tsrpcBaseClient.getCustomObjectIdTypes(getClassObjectId())
        }, {
    ...
    async _onApiCall(call) {
        ...
        call = preFlow;
        // exec ApiCall
        call.logger.log(chalk__default["default"].green('[ApiReq]'), this.options.logReqBody ? call.req : '');
    ...
    async _onMsgCall(call) {
        ...
        call = preFlow;
        // MsgHandler
        this.options.logMsg && call.logger.log(chalk__default["default"].green('[RecvMsg]'), call.msg);
        let promises = [
            // Conn Handlers
    ...
    _parseServerInput(tsbuffer, serviceMap, data, serviceId) {
        var _a;
        if (data instanceof Uint8Array) {
            ...
            if (serviceId == undefined) {
                ...
                sn = json[2];
                // Parse serviceId
                const serviceName = json[0];
                const isMsg = sn === undefined;
                serviceId = (_a = (isMsg ? serviceMap.msgName2Service : serviceMap.apiName2Service)[serviceName]) === null || _a === void 0 ? void 0 : _a.id;
                if (serviceId === undefined) {
            ...
            if (!service) {
                let errMsg = `Invalid service id: ${chalk__default["default"].cyan.underline(serviceId)}`;
...
class HttpServer extends BaseServer {
    ...
    start() {
        ...
        return new Promise(rs => {
            ...
            this.httpServer = (this.options.https ? https__default["default"] : http__namespace).createServer({
                ...this.options.https
            }, (httpReq, httpRes) => {
                var _a, _b;
                ...
                httpReq.on('end', async () => {
                    ...
                    if (conn.dataType === 'text') {
                        ...
                        let serviceId;
                        if (isMsg) {
                            serviceId = (_a = this.serviceMap.msgName2Service[serviceName]) === null || _a === void 0 ? void 0 : _a.id;
                        }
                        else {
                            serviceId = (_b = this.serviceMap.apiName2Service[serviceName]) === null || _b === void 0 ? void 0 : _b.id;
                        }
                ...
                httpRes.on('close', async () => {
                    ...
                    // 客户端Abort
                    if (httpReq.aborted) {
                        ((_b = (_a = conn.call) === null || _a === void 0 ? void 0 : _a.logger) !== null && _b !== void 0 ? _b : conn.logger).log('[ReqAborted]');
                    }
                    ...
                    // 有Conn，但连接非正常end：直到连接关闭，也未调用过 httpRes.end 方法
                        ...
                        (((_c = conn.call) === null || _c === void 0 ? void 0 : _c.logger) || conn.logger).warn('Socket closed without response');
                    }
                    // Post Flow
...
class WsConnection extends BaseConnection {
    constructor(options) {
        super(options, new PrefixLogger({
            logger: options.server.logger,
            prefixs: [chalk__default["default"].gray(`${options.ip} Conn#${options.id}`)]
        }));
        this.type = "LONG";
        ...
        if (this.server.options.heartbeatWaitTime) {
            ...
            this._heartbeatInterval = setInterval(() => {
                if (Date.now() - this._lastHeartbeatTime > timeout) {
                    this.logger.debug('Receive heartbeat timeout');
                    this.close('Receive heartbeat timeout', 3001, 1000);
                }
            }, timeout);
        }
        // Init WS
    ...
    close(reason, code = 1000, closeTimeout = 3000) {
        // 停止心跳
        if (this._heartbeatInterval) {
            clearInterval(this._heartbeatInterval);
            this._heartbeatInterval = undefined;
        ...
        return new Promise(rs => {
            ...
            // 超时保护
            setTimeout(() => {
                if (ws.readyState !== WebSocket__namespace.CLOSED) {
                    ws.terminate();
                }
            }, closeTimeout);
        }).finally(() => {
...
class WsServer extends BaseServer {
    constructor(proto, options) {
        ...
        this._onClientConnect = (ws, httpReq) => {
            ...
            this.options.logConnect && conn.logger.log(chalk__default["default"].green('[Connected]'), `ActiveConn=${this.connections.length}`);
            this.flows.postConnectFlow.exec(conn, conn.logger);
        };
        this._onClientClose = async (conn, code, reason) => {
            this.connections.removeOne(v => v.id === conn.id);
            delete this._id2Conn[conn.id];
            this.options.logConnect && conn.logger.log(chalk__default["default"].green('[Disconnected]'), `Code=${code} ${reason ? `Reason=${reason} ` : ''}ActiveConn=${this.connections.length}`);
    ...
    start() {
        ...
        return new Promise((rs, rj) => {
            ...
            this._httpServer = (this.options.wss ? https__default["default"] : http__namespace).createServer({
                ...this.options.wss
            });
    ...
    async stop() {
        ...
        return new Promise(async (rs, rj) => {
            ...
            // Close HTTP Server
            await new Promise((rs1, rj1) => { var _a; (_a = this._httpServer) === null || _a === void 0 ? void 0 : _a.close(err => { err ? rj1(err) : rs1(); }); });
            // Close WS Server
