    ...
    private async doMatch(e: ServerMatch) {
        ...
        for (let call of queue) {
            ...
            // 没有合适的房间，那么创建一个房间
                ...
                let ret = await MatchUtil.createRoom('系统房间' + (NextRoomIndex++));
                if (ret.isSucc) {
                    matchingRooms.push({
                        id: ret.res.room_info.roomId,
                        area_number: ret.res.room_info.area_number,
                        serverUrl: ret.res.room_info.serverUrl,
                        playerNum: 1
                    })
