...
exports.ProtoParse = void 0;
const messageConfig_1 = require("./messageConfig");
/**网络管理器 */
class ProtoParse {
    static onMessage(client_ws, message) {
        /**将数据转成arraybuffer */
        let u8_array = new Uint8Array(message);
        ...
        /**解析数据 */
        let config = messageConfig_1.MessageConfig[type];
        ...
        /**以下为测试代码 */
        if (type == 2 /* MessageType.C2SHello */) {
            ProtoParse.send(client_ws);
        }
    }
    static send(client_ws) {
        let config = messageConfig_1.MessageConfig[2 /* MessageType.C2SHello */];
        /**使用pb编码数据 */
...
exports.ProtoParse = ProtoParse;
/**数据头+数据的总长度 所占字节数 */
ProtoParse.first_length = 2;
/**c2s数据头所占字节数 */
ProtoParse.c2s_head_length = 3;
/**s2c数据头所占字节数 */
ProtoParse.s2c_head_length = 1;
/**是否为小端存储数据 */
ProtoParse.little_endian = 0x01020304 >> 24 == 4;
